classdef json_decoder < handle
    % v = json_decoder.parse(jsonString) converts a json_decoder string to a MATLAB value.
    %
    % This started out as a recursive descent parser, but json_decoder is so simple
    % that most of the parser collapsed out.
    %
    % In the service of speed, simplicity, and laziness, this code is NOT a
    % validator. Its purpose is to convert correct json_decoder strings to MATLAB
    % values. It does not reject all malformed json_decoder.
    ...
    methods (Access = private)
        ...
        function this = json_decoder(JSONstring)
        ...
        function array = getArray(this)
            ...
            while ~strcmp(value,']')
                ...
                elseif strcmp(value,']')
                    ...
                    error('json_decoder parser requires commas between array elements');
                end
            end
        ...
        function obj = getObject(this)
            ...
            while ~strcmp(value,'}')
                ...
                value = this.getValue;
                if ~strcmp(value,':')
                    error('json_decoder parser requires colons between object names and values');
                end
                ...
                elseif strcmp(value,'}')
                    ...
                    error('json_decoder parser requires commas between object elements');
                end
            end
        ...
        function [token,tokenType] = getNextToken(this)
            ...
            function match(this,str)
                ...
                if strcmp(str,found)
                    ...
                    error('The json_decoder parser expected "%s" but found %s',str,found)
                end
            end
            ...
            function tf = isSpecial(aChar)
                % the special characters in the json_decoder "language"
    ...
    methods(Static)
        ...
        % json_decoder.parse(string)... that should be familiar to Javascrpt
        % programmers
        function value = parse(JSONstring)
            jsonObject = json_decoder(JSONstring);
